# M01-T4: Remove the duplicate file and sync the example manifests

- **Class:** mechanical
- **Status:** ready
- **Depends:** — (independent; strongly complements M01-T1/T2)
- **Issues:**
...
inputs. If M01-T2 has already replaced the hand-written manifest with a
...
- **Out:** the CMake migration (T1/T2); charts (M02); branding SPDX
  cleanup (that's M08-T4/I-0019, though fixing the `.pro` path here
...
1. Delete the diverged working-copy variant (I-0013). Review the diff
   ...
   looks like a wanted direction, note that in the M02-T1 packet; either
   ...
   qmldir is still live, i.e. M01-T1 hasn't migrated the module yet):
   ...
   are WebEngine-based and are removed by M02-T2.
...
5. Add a **CI guard** (a small script, wired in M04-T1/CI) that fails
...
- The qmake WASM build finds all its inputs (or M01-T2 has superseded
...
half of I-0001 met; `status.yml` M01-T4 → done.
