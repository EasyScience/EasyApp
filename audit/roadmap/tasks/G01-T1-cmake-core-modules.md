# G01-T1: CMake `qt_add_qml_module` for the core modules (Style, Globals, Elements)
...
- **Out:** migrating Components/Charts/Logic (that's G01-T2); replacing
  WebEngine charts (G02); the PySide packaging change (G06) — but **do
...
7. Run `qmllint` on the three modules; fix module-identity warnings
   ...
   warnings for G05 (do **not** fix those here).
...
`status.yml` G01-T1 → done and G01-T2 unblocked.
