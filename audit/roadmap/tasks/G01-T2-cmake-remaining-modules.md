# G01-T2: Migrate the remaining modules to CMake; retire the hand-written `.qrc`/`.pro`

- **Class:** standard
- **Status:** blocked (needs G01-T1)
- **Depends:** G01-T1
- **Issues:**
...
- **Anchors:**
  [architecture-target.md](../../design/architecture-target.md); G01-T1
  result

## Goal

Extend the G01-T1 CMake build to the remaining modules (`Components`,
`Charts` — the WASM-capable subset only, `Logic`, `Logic/Maintenance` if
it still exists post-G03) so the **whole app** builds and renders on
...
- **Out:** the WebEngine charts — **exclude** them from the WASM target
  (they cannot compile/run there); the native chart façade is G02-T1.
  Gate the WebEngine chart files behind a desktop-only CMake condition
  or omit until G02 replaces them.

## Plan

1. Add module targets mirroring G01-T1 for `Components`, `Logic`; for
...
3. Replace the `BasicC++` example with a CMake target that links the
   ...
   (a minimal host page model — see G03-T1).
4. Delete `resources.qrc`, `add_aliases_to_qrc.py`, `*.pro`.
5. Point G01-T3 CI at `qt-cmake --preset wasm`.
6. Run `qmllint` across all migrated modules; record (don't fix)
   private-`.impl` warnings for G05.
...
I-0004 complete; I-0001/I-0020 structurally closed; G02 unblocked;
`status.yml` updated.
