 ...
 * https://github.com/kromain/qml-utils/tree/master/JSONListModel
...
ListModel {
    ...
    property int status: JsonListModel.Null
    enum StatusEnum {
        Null,
        Ready,
        Updating,
    ...
    onSourceChanged: updateJson()
    onJsonChanged: updateListModel()
    onQueryChanged: updateListModel()
    ...
    function updateListModel() {
        listModel.status = JsonListModel.Updating
        ...
        let objectArray = parseJsonString(listModel.json, listModel.query)
        for (let key in objectArray) {
            const jo = objectArray[key]
            listModel.append(jo)
        }

        listModel.status = JsonListModel.Ready
    }
